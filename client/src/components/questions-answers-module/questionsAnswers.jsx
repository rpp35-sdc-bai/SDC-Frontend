...
class QuestionsAnswers extends React.Component {
   ...
   addShownQuestions() {
      ...
      currShownQuestions = currShownQuestions.concat(
         currAllQuestions.splice(0, 2)
      );
      this.setState({
         shownQuestions: currShownQuestions,
         allQuestions: currAllQuestions,
      });
   }
   ...
   getQuestions() {
      axios
         .get(`${__API__}/questionsAnswers/questions`, {
            params: { product_id: this.props.currentProductId },
         })
         .then((results) => {
            this.setState({
               shownQuestions: results.data.results.splice(0, 2),
   ...
   getProductInfo() {
      axios
         .get(`${__API__}/questionsAnswers/productInfo`, {
            params: { product_id: this.props.currentProductId },
         })
         .then((results) => {
   ...
   handleInteraction(event) {
      ...
      axios
         .post('/questionsAnswers/interactions', {
            interaction: data,
         })
         .then((results) => {
      ...
      return (
         ...
         >
            <Typography
               align='center'
               variant='h3'
               className='questionsAnswersTile'
            >
               Questions & Answers
            </Typography>
            <Search
            ...
            <div className='QA_body'>
               <QuestionsList
                  ...
                  handleInteraction={this.handleInteraction}
                  questions={
                     this.state.filteredQuestions !== undefined
                        ? this.state.filteredQuestions
                        : this.state.shownQuestions
                  }
